		...
		<div style={{
					...
					filteredServices.map(service => (
						...
						>
							<div className="service-header">
								<h3>{formatTo12h(service.startHour)} - {formatTo12h(service.endHour)} | {formatServices(service.services) || 'Servicio'}</h3>
			...
			{filteredServices.length > 0 ? (
				<div className="busqueda-form-buttons busqueda-full-width" style={{ marginBottom: '10px' }}>
					<button type="reset" className="cancel-btn" onClick={onNuevaBusqueda}>
						❌ CANCELAR
					</button>
				</div>
			) : (
